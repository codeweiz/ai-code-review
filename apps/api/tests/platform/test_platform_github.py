...
class TestPlatformGithub(unittest.TestCase):
    """测试 GitHub 平台接口"""
    ...
    def setUp(self):
        ...
        self.test_file_name = "README"
        self.test_ref = "7044a8a032e85b6ab611033b2ac8af7ce85805b2"

    def test_request(self):
        """测试访问 GitHub 的仓库信息"""
        github_platform = get_git_platform(platform_name=PLATFORM_GITHUB)
        request = GitPlatformRequest(
            method=REST_METHOD_GET,
            endpoint=f"/repos/{self.test_owner}/{self.test_repo}",
        )
        resp = github_platform.request(request)
        logger.info(resp)

    def test_get_pr_info(self):
        """测试获取 Git Pull Request 信息"""
        github_platform = get_git_platform(platform_name=PLATFORM_GITHUB)
        resp = github_platform.get_pr_info(
    ...
    def test_get_pr_files(self):
        ...
        github_platform = get_git_platform(platform_name=PLATFORM_GITHUB)
        resp = github_platform.get_pr_files(
            repo_name=f"{self.test_owner}/{self.test_repo}",
            pr_number=self.test_pr_number,
        )
        logger.info(resp)
    ...
    def test_get_pr_commits(self):
        ...
        github_platform = get_git_platform(platform_name=PLATFORM_GITHUB)
        resp = github_platform.get_pr_commits(
            repo_name=f"{self.test_owner}/{self.test_repo}",
            pr_number=self.test_pr_number,
        )
        logger.info(resp)
    ...
    def test_get_file_content(self):
        """测试获取文件内容"""
        github_platform = get_git_platform(platform_name=PLATFORM_GITHUB)
        resp = github_platform.get_file_content(
            repo_name=f"{self.test_owner}/{self.test_repo}",
    ...
    def test_get_webhooks(self):
        """测试获取 webhook 列表"""
        github_platform = get_git_platform(platform_name=PLATFORM_GITHUB)
        resp = github_platform.get_webhooks(repo_name="codeweiz/pulse-guard")
        logger.info(resp)
